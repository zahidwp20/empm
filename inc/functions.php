...
if (!function_exists('empm_update_option')) {
    /**
     * Update or add option in settings table
     *
     * @param $option_key
     * @param $option_value
     *
     * @return bool
     */
    function empm_update_option($option_key, $option_value = '')
    {
        $conn = empm_get_var('conn');
        ...
        $sql_search = "SELECT count(option_key) FROM " . EMPM_TBL_OPTIONS . " WHERE `option_key` = '$option_key'";
        $search_result = $conn->query($sql_search);
        $search_result = $search_result->fetch_assoc();
        $search_result = (int)reset($search_result);

        // Update option key if that key is exists
        if ($search_result > 0) {
            $sql_update = "UPDATE " . EMPM_TBL_OPTIONS . " SET `option_value` = '$option_value' WHERE `option_key` = '$option_key'";
        ...
        // Insert option key and value if that option is not exists
        $sql_insert = "INSERT INTO " . EMPM_TBL_OPTIONS . "(option_key,option_value) VALUES ('$option_key','$option_value');";
...
function empm_get_option($option_key = '')
{
    $conn = empm_get_var('conn');
    ...
    $get_data_sql = "SELECT (option_value) FROM " . EMPM_TBL_OPTIONS . " WHERE `option_key` = '$option_key' LIMIT 1";
    $get_result = $conn->query($get_data_sql);
    $get_result = $get_result->fetch_assoc();

    return empm_get_var('option_value', $get_result);
}


function selected($selected, $current)
{
    echo $selected === $current ? 'selected' : '';
}


function empm_get_timezones()
{
    return array(
